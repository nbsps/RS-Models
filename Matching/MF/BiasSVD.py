...
# declare MF model
class BiasSVDModel(nn.Module):
    def __init__(self, user_num, item_num, mean, k=10):
        super(BiasSVDModel, self).__init__()
    ...
    def forward(self, uid, mid):
        ...
        return (self.u_emb(uid) * self.i_emb(mid)).sum(1) + u_bias + i_bias + self.mean
...
def main():
    ...
    model = BiasSVDModel(user_num, item_num, mean, k=20).to(device)
