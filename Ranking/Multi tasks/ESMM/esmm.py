...
class CensusDataset(Dataset):
    def __init__(self, data):
        ...
        self.ctr = data['marital_status'].values
        self.ctcvr = data['ctcvr'].values

    def __getitem__(self, index):
        return self.categorical_features[index], self.continuous_features[index], self.ctr[index], self.ctcvr[index]
...
class ESMM(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # task1: ctr task2: ctcvr
        task_outs.append(task_outs[0] * task_outs[1])
        return task_outs
...
def data_generate():
    ...
    data['ctcvr'] = data['income_50k'] * data['marital_status']
...
if __name__ == "__main__":
    ...
    esmm = ESMM(concrete_features, continuous_num, emb_dim=EMBEDDING_DIM,
                hidden_units=HIDDEN_UNITS, experts_units=EXPERT_UNITS).to(device)
    optimizer = torch.optim.Adam(esmm.parameters(), lr=LEARNING_RATE, weight_decay=REGULARIZATION)
    ...
    for i in range(EPOCH):
        ...
        for x1, x2, y1, y2 in train_dataloader:
            ...
            predict[2] = predict[2].squeeze()
            loss_1 = loss_func(predict[1], y1)
            loss_2 = loss_func(predict[2], y2)
            loss = loss_1 + loss_2
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        for x1, x2, y1, y2 in test_dataloader:
            ...
            income_predict += predict[1]
            marry_predict += predict[2]
            loss_1 = loss_func(predict[1], y1)
            loss_2 = loss_func(predict[2], y2)
            loss = loss_1 + loss_2
        ...
        print("epoch {}, train loss is {:.4f}, marry(ctr) auc is {:.4f}, "
              "ctcvr auc is {:.4f}".format(i + 1, train_loss, income_auc, marry_auc))

    print("max auc in test dataset: ctr - {:.4f}, ctcvr - {:.4f}".
